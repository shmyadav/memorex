...
def get_entity_node_save_query(labels: str, has_aoss: bool = False) -> str:
...
def get_entity_node_save_bulk_query(nodes: list[dict], has_aoss: bool = False) -> str:
    save_embedding_query = (
...
def get_entity_node_return_query(_provider: GraphProvider | None = None) -> str:
...
def get_entity_edge_return_query(_provider: GraphProvider | None = None) -> str:
    return """
        e.uuid AS uuid,
        e.group_id AS group_id,
...
def _ensure_neo4j(driver: GraphDriver) -> None:
    """
    This codebase is intended to run against Neo4j only.

    Historically, `search_utils.py` contained compatibility branches for other graph providers.
    We now hard-enforce Neo4j to keep the search implementation predictable and easy to maintain.
    """
    # if driver.provider != GraphProvider.NEO4J:
    #     raise NotImplementedError(
    #         f'Only Neo4j is supported by this project. Got provider={driver.provider}.'
    #     )
    pass
...
async def get_mentioned_nodes(
    ...
    nodes = [EntityNode.get_entity_node_from_record(record) for record in records]
...
async def get_relevant_nodes(
    ...
    relevant_nodes_dict: dict[str, list[EntityNode]] = {
        result['search_node_uuid']: [
            EntityNode.get_entity_node_from_record(record) for record in result['matches']
        ]
